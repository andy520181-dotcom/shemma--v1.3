...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  // 显示自定义HTML Action Sheet（不使用iOS原生选择器）
  const handleAvatarClick = () => {
    console.log('=== 头像点击事件触发 ===');
    // 显示我们自定义的HTML Action Sheet
    setShowMenu(true);
  };
  ...
  const handleCamera = async () => {
    ...
    console.log('开始调用相机...');
    try {
      // 直接指定source为Camera，不让系统弹出选择器
      const photo = await Camera.getPhoto({
        quality: 90,
        source: CameraSource.Camera,
        resultType: CameraResultType.DataUrl,
        // 不设置 promptLabelHeader 等参数，避免触发原生UI
      });
  ...
  // 从相册选择 - 直接打开相册，不显示原生选择器
  const handleLibrary = async () => {
    setShowMenu(false);
    console.log('开始打开相册...');
    try {
      // 直接指定source为Photos，不让系统弹出选择器
      const photo = await Camera.getPhoto({
        quality: 90,
  ...
  return (
    ...
    >
      ...
      <div
        className="pt-[38px] pb-2 w-full flex justify-center items-center z-20 shrink-0"
        style={{
      ...
      <div
        className="flex-1 flex flex-col relative pb-2"
        style={{
          minHeight: '0',
          paddingLeft: 0,
          paddingRight: 0,
      ...
      >
        ...
        <div
          className="w-full flex flex-col items-start mt-4 pl-1 relative z-10 shrink-0"
          style={{
            paddingLeft: '2rem',
            paddingRight: '2rem'
          }}
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center w-full">
            {/* Fixed 280px Container - 固定尺寸确保布局稳定并与结果页一致 - 使用负margin强制偏移 */}
            <div
            ...
            >
              ...
              >
                ...
                <div
                  className="w-full h-full rounded-full flex items-center justify-center overflow-hidden border-[8px] border-gray-50 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] transition-all duration-500 ease-out"
                  style={{
                    borderWidth: '8px',
                    borderColor: '#F9FAFB',
                    borderStyle: 'solid',
      ...
      {showMenu && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center">
          ...
          />
          <div
            className="relative z-10 w-full max-w-[480px] bg-[#F2F2F7] rounded-t-2xl px-4 py-3 space-y-3 animate-in slide-in-from-bottom duration-300"
            style={{
          ...
          >
            ...
            >
              <button
                ...
                className="w-full h-[56px] flex items-center justify-center text-[14px] font-normal text-black active:bg-gray-100 transition-colors border-b border-gray-100/50"
              ...
              <button
                ...
                className="w-full h-[56px] flex items-center justify-center text-[14px] font-normal text-black active:bg-gray-100 transition-colors"
                style={{ backgroundColor: '#FFFFFF' }}
              >
                从相册选择
              </button>
